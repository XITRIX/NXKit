...
| `UIEvent`, `UITouch`, presses and keys | Partial | Touch and gamepad/press events exist. Complete timestamps, phases, multi-touch identity, cancellation, modifier/key semantics, mouse input, and deterministic tests. |
| `UIView` geometry and hierarchy | Partial | Frame/bounds/center, subviews, superview, hit testing, conversion, alpha, hidden state, backgrounds, masks, tint and content modes exist. Fix index validation, hierarchy-cycle checks, mask rendering, invalidation, conversion through transforms, and insertion parity. |
...
| `CALayer` geometry, hierarchy, and visual properties | Partial | Bounds, position, anchor point, affine/3D transform, z-order, opacity, background, rounded corners, borders, shadows, contents, hierarchy, and presentation copies exist. Fix incomplete insertion, mask behavior, invalidation, contents rectangles/centers, coordinate conversion, and safe hierarchy mutation. |
...
| Layer masking | Stub/unsafe | `CALayer::mask` is stored and included in hierarchy bookkeeping but ignored by rendering. Implement alpha/luminance masking and define transform/bounds behavior. |
