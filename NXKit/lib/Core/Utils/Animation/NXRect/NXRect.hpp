...
class NXRect : public FiniteTicking
{
public:
    /**
     * Creates an animatable with the given initial value.
     */
    NXRect(Rect value = Rect());
     ...
     */
    void addStep(Rect targetValue, int32_t duration, EasingFunction easing = EasingFunction::linear);
    ...
    operator Rect() const;
    operator Rect();
    void operator=(const Rect value);
    bool operator==(const Rect value);
